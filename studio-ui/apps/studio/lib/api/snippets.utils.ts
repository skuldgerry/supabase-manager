 ...
 */
let _warnedMissingSnippetsDir = false

export function warnSnippetsDirMissing() {
  if (!_warnedMissingSnippetsDir) {
    _warnedMissingSnippetsDir = true
    console.warn(
      '[snippets] SNIPPETS_MANAGEMENT_FOLDER is not set — snippet storage is disabled. ' +
        'Set this env var to a directory path to enable saved SQL snippets.'
    )
  }
}

export function isSnippetStorageConfigured() {
  return SNIPPETS_DIR !== ''
}
...
/**
 * Renames a folder directory on the filesystem
 * @throws {Error} If the folder doesn't exist or the new name is already taken
 */
export async function renameFolder(id: string, newName: string): Promise<Folder> {
  const folderName = sanitizeName(newName)
  const entries = await getFilesystemEntries()
  ...
  if (folder.name !== folderName) {
    const existing = entries.find((f) => f.name === folderName && f.type === 'folder')
    if (existing) {
      throw new Error(`Folder with name ${folderName} already exists`)
    }
  }

  const oldPath = path.join(SNIPPETS_DIR, folder.name)
  const newPath = path.join(SNIPPETS_DIR, folderName)

  await fs.rename(oldPath, newPath)
  return buildFolder(folderName)
}
