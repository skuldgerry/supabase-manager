...
function useConnectionStringPooler(): ConnectionStringPooler {
  ...
  // Build session URIs by replacing the transaction pooler port with the session port.
  // On cloud, Supavisor uses 6543 for transaction and 5432 for session.
  // On self-hosted, there's no separate session port — substitute the direct postgres port
  // so each project gets its own correct port rather than always replacing 6543.
  const transactionPort = poolingConfigurationShared?.db_port ?? 6543
  const sessionPort = connectionInfo.db_port || 5432

  return useMemo(
    () => ({
      transactionShared: connectionStringsShared.pooler.uri,
      sessionShared: connectionStringsShared.pooler.uri.replace(
        `:${transactionPort}`,
        `:${sessionPort}`
      ),
    ...
    [connectionStringsShared, connectionStringsDedicated, ipv4Addon, transactionPort, sessionPort]
  )
}
