...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // If Logflare isn't configured, return graceful empty responses instead of 500
  if (missingEnvVars !== true || !baseUrl) {
    if (method === 'GET') return res.status(200).json([])
    return res.status(503).json({ error: { message: 'Log drains are not configured on this instance.' } })
  }
  ...
  switch (method) {
    case 'GET':
      ...
      try {
        ...
        // Logflare unreachable — return empty list so the UI doesn't break
